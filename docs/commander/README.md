...
`--all-reviewed` compiles all 1,170 currently source-bound primary definitions, 20 auxiliary token templates and 46 legal development decks, including 97 temporary creature modifiers, 232 exact keyword-reminder bodies, seven counterspells, five creature-return spells and 27 fixed token producers and 41 static-bonus permanents (38 currently have legal executed source fixtures; Arvad closes the prior Call to the Feast gap). Source admission verifies every definition against the closed authenticated registry. Earlier constructor flags remain available for their narrower development subsets.
