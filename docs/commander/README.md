...
`--all-reviewed` compiles all 1,129 currently source-bound primary definitions, 20 auxiliary token templates and 40 legal development decks, including 97 temporary creature modifiers, 232 exact keyword-reminder bodies, seven counterspells, five creature-return spells and 27 fixed token producers (26 currently have legal executed source fixtures). Source admission verifies every definition against the closed authenticated registry. Earlier constructor flags remain available for their narrower development subsets.
