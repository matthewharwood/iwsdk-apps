...
for (const card of cards) {
  ...
  const creatureFaces = faces
    .map((face, index) => ({ face, index }))
    .filter(({ face }) => /\bCreature\b/.test(face.type_line ?? ""));
  ...
  for (const { face, index } of creatureFaces) {
    ...
    const prefixes = [
      ...
      `When ${face.name} enters, `,
      `When ${face.name} enters the battlefield, `,
    ];
    const effects: {
      line: number;
      kind: "draw" | "gain-life";
    ...
    for (const [lineNumber, line] of lines.entries()) {
      ...
      const draw =
        /^(?:you (may )?)?draw (a|one|two|three|four|five|six|seven|eight|nine|ten) cards?\.$/.exec(
          body,
        );
    ...
    if (
      ...
      failures.push("unreviewed-characteristic-type");
    ...
    const disposition = failures.length
      ...
      : exact?.optional
        ? "optional-requires-resolution-choice"
        : "mandatory-research-candidate";
    const unresolvedClues = [
      ["optional-resolution-choice", /\bmay\b/],
      ["conditional-or-variable-effect", /\bif\b|\bunless\b|\bfor each\b|\bequal to\b|\bX\b/],
      ["target-domain", /\btarget\b/],
      ["generated-or-external-definition", /\bcreate\b|\bcopy\b|\btoken/],
      ["additional-zone-transition", /\bdiscard\b|\breturn\b|\bexile\b|\bsacrifice\b/],
      ["delayed-or-linked-trigger", /\bnext\b|\bwhen you do\b/i],
    ...
    records.push({
      ...
      failures,
      dependencies: {
        externalDefinitions: failures.length ? null : [],
        requiredCore: [
          "committed-zone-events",
          "effective-ability-snapshot",
          "pending-trigger-queue",
          "apnap-placement",
          "controller-order-decision",
        ...
        clues: unresolvedClues.filter(([, pattern]) => pattern.test(text)).map(([label]) => label),
        assessment: failures.length
          ? "unresolved"
          : "candidate-constant-effects-no-external-definitions; requires-semantic-review",
      },
...
const ruleSources = ruleIds.map((id) => {
  ...
  if (!row) throw new Error(`Missing rule ${id}`);
  return row;
});
const mandatory = records.filter((row) => row.disposition === "mandatory-research-candidate"),
  optional = records.filter((row) => row.disposition === "optional-requires-resolution-choice");
const families = [
  ...new Set(
    [...mandatory, ...optional].map(
      (row) =>
        `${row.proposedEffects[0]?.kind}:${row.proposedEffects[0]?.amount}:${row.proposedEffects[0]?.optional ? "optional" : "mandatory"}`,
    ),
  ),
]
  .sort()
  .map((family) => ({
    family,
    count: [...mandatory, ...optional].filter(
      (row) =>
        `${row.proposedEffects[0]?.kind}:${row.proposedEffects[0]?.amount}:${row.proposedEffects[0]?.optional ? "optional" : "mandatory"}` ===
        family,
    ).length,
...
const base = {
  ...
  mandatoryCandidates: mandatory.length,
  optionalCandidates: optional.length,
  rejectedBodyOrCharacteristics: records.length - mandatory.length - optional.length,
  families,
  records,
  ruleSources,
  ...
  JSON.stringify(
    {
      ...
      families,
      mandatory: mandatory.map((row) => ({
        name: row.name,
        program: row.proposedEffects,
        keywords: row.otherAbilities.keywords,
      })),
      optional: optional.map((row) => ({ name: row.name, text: row.completeOracleText })),
    },
    null,
    2,
  ),
