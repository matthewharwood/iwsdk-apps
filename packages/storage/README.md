...
The [engine 0.7 trigger persistence proof](../../docs/commander/trigger-storage-proof-0.7.md) separates synthetic ordinary commands, constructed simultaneous-entry ordering, and source-backed full games. `--require-triggers` adds a real pending-ability reload stage and requires actual emitted ability-resolution events. The standalone constructed proof is run with `bun packages/storage/browser-proof/trigger-scenario-check.ts`; its test-only initializer is excluded from production exports.
