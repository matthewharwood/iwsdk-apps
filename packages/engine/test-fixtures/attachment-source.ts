...
import snapshot from "./attachment-source.json";
import { tokenSourcePayment } from "./token-source";

export { snapshot as attachmentSourceSnapshot };
...
export function attachmentSourceCard(name: string): CardDefinition {
  const card = Object.values(definitions).find((card) => card.name === name);
  if (!card) throw new Error(`Missing authenticated attachment fixture card: ${name}`);
  return card;
}
export async function attachmentSourceRelease(): Promise<ContentRelease> {
  const body = {
    schema: "commander-content/1" as const,
    id: "authenticated-attachment-source-fixture/1",
    ...
    compilerVersion: "authenticated-attachment-source-fixture/1",
...
function sourceRegistry() {
  admitted ??= attachmentSourceRelease().then(async (release) => ({
    release,
    registry: await createFullExecutionRegistry(release),
  }));
  return admitted;
}
export async function attachmentSourceDeck(
  commander: string,
  names: string[],
): Promise<DeckRevision> {
  const leader = attachmentSourceCard(commander);
  ...
  const body = {
    id: `attachment-source:${commander}:${(await semanticHash(names)).slice(0, 16)}`,
    commander: leader.id,
    entries: [
      ...unique.map((name) => ({ definition: attachmentSourceCard(name).id, count: 1 })),
      ...basics.map((name, index) => ({
        definition: attachmentSourceCard(name).id,
...
export type AttachmentSourceFixture = {
  state: RulesState;
  registry: ExecutionRegistry;
  release: ContentRelease;
  source: string;
  history: { command: GameCommand; revision: number; response: Response; events: GameEvent[] }[];
  lands: Record<string, Partial<Record<"W" | "U" | "B" | "R" | "G" | "C", string[]>>>;
};
export function attachmentSourceObject(f: AttachmentSourceFixture, actor: string, name: string) {
  const entry = Object.values(f.state.objects).find(
    (entry) => entry.owner === actor && entry.definition === attachmentSourceCard(name).id,
  );
  if (!entry) throw new Error(`Missing owned source card ${actor}/${name}`);
  return entry;
}
export function attachmentSourceCommand(
  f: AttachmentSourceFixture,
  response: Response,
): GameCommand {
...
export function attachmentSourceAnswer(f: AttachmentSourceFixture, response: Response) {
  const command = attachmentSourceCommand(f, response);
...
export async function attachmentSourceFixture(
  commander: string,
  names: string[],
  ...
  seats: 2 | 4 = 2,
): Promise<AttachmentSourceFixture> {
  const { release, registry: full } = await sourceRegistry();
  const decks = await Promise.all([
    attachmentSourceDeck(commander, names),
    attachmentSourceDeck(opponent, otherNames),
  ]);
  ...
  const f: AttachmentSourceFixture = {
    release,
    registry,
    source: "",
    history: [],
    ...
    state: createMatch(
      {
        ...
        id: "authenticated-attachment-scenario",
        ...
        gameSeed: 4,
        driverSeed: 1,
        driverVersion: "authenticated-attachment-scenarios/1",
  ...
  attachmentSourceAnswer(f, { kind: "starting-player", player: "A" });
  while (f.state.decision?.kind === "mulligan")
    attachmentSourceAnswer(f, { kind: "mulligan", keep: true });
  for (let n = 0; n < 16 && f.state.step !== "main1"; n++)
    attachmentSourceAnswer(f, { kind: "pass" });
  ...
  for (const [actor, hand] of [
    ...
    for (const name of hand) {
      const entry = attachmentSourceObject(f, actor, name);
      if (entry.zone !== "hand" && !entry.commander)
        move(f.state, entry.id, "hand", "constructed authenticated source hand");
    }
    const kept = new Set(hand.map((name) => attachmentSourceCard(name).id));
    ...
    const lands: AttachmentSourceFixture["lands"][string] = {};
...
export function attachmentSourceCast(
  f: AttachmentSourceFixture,
  actor: string,
  name: string,
  target?: string,
  ...
  const before = attachmentSourceObject(f, actor, name);
  attachmentSourceAnswer(f, { kind: "cast", card: before.id });
  if (target) attachmentSourceAnswer(f, { kind: "target", target });
  ...
  const paid = attachmentSourceAnswer(f, payment);
...
export function attachmentSourceResolve(f: AttachmentSourceFixture) {
  const top = f.state.stack.at(-1);
...
export function attachmentSourceDrain(f: AttachmentSourceFixture) {
  for (
    let n = 0;
  ...
  if (f.state.stack.length || f.state.decision?.kind !== "priority")
    throw new Error("Attachment scenario did not finish all mandatory abilities");
}
export function attachmentSourceInventory(f: AttachmentSourceFixture, actor: string) {
...
export function attachmentSourceMain(f: AttachmentSourceFixture, actor: string): void {
  for (let n = 0; n < 300; n++) {
        ...
        : d.kind === "trigger-payment"
          ...
          : d.kind === "attack"
            ...
            : d.kind === "discard"
              ...
              : { kind: "pass" };
    attachmentSourceAnswer(f, response);
  }
