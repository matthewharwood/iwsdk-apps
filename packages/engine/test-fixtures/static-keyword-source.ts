...
import snapshot from "./static-keyword-source.json";
import { tokenSourcePayment } from "./token-source";

export { snapshot as keywordSourceSnapshot };
...
export function keywordSourceCard(name: string): CardDefinition {
  const card = Object.values(definitions).find((card) => card.name === name);
  if (!card) throw new Error(`Missing authenticated keyword-grant fixture card: ${name}`);
  return card;
}
export async function keywordSourceRelease(): Promise<ContentRelease> {
  const body = {
    schema: "commander-content/1" as const,
    id: "authenticated-keyword-grant-source-fixture/1",
    ...
    compilerVersion: "authenticated-keyword-grant-source-fixture/1",
...
function sourceRegistry() {
  admitted ??= keywordSourceRelease().then(async (release) => ({
    release,
    registry: await createFullExecutionRegistry(release),
  }));
  return admitted;
}
export async function keywordSourceDeck(commander: string, names: string[]): Promise<DeckRevision> {
  const leader = keywordSourceCard(commander);
  ...
  const body = {
    id: `keyword-source:${commander}:${(await semanticHash(names)).slice(0, 16)}`,
    commander: leader.id,
    entries: [
      ...unique.map((name) => ({ definition: keywordSourceCard(name).id, count: 1 })),
      ...basics.map((name, index) => ({
        definition: keywordSourceCard(name).id,
...
export type KeywordSourceFixture = {
  state: RulesState;
...
export function keywordSourceObject(f: KeywordSourceFixture, actor: string, name: string) {
  const entry = Object.values(f.state.objects).find(
    (entry) => entry.owner === actor && entry.definition === keywordSourceCard(name).id,
  );
  if (!entry) throw new Error(`Missing owned source card ${actor}/${name}`);
  return entry;
}
export function keywordSourceCommand(f: KeywordSourceFixture, response: Response): GameCommand {
...
export function keywordSourceAnswer(f: KeywordSourceFixture, response: Response) {
  const result = transition(f.state, keywordSourceCommand(f, response), f.registry);
...
export async function keywordSourceFixture(
  commander: string,
  names: string[],
  ...
  seats: 2 | 4 = 2,
): Promise<KeywordSourceFixture> {
  const { release, registry: full } = await sourceRegistry();
  const decks = await Promise.all([
    keywordSourceDeck(commander, names),
    keywordSourceDeck(opponent, otherNames),
  ]);
    ...
    state: createMatch(
      {
        ...
        id: "authenticated-keyword-scenario",
        ...
        gameSeed: 4,
        driverSeed: 1,
        driverVersion: "authenticated-keyword-scenarios/1",
  ...
  keywordSourceAnswer(f, { kind: "starting-player", player: "A" });
  while (f.state.decision?.kind === "mulligan")
    keywordSourceAnswer(f, { kind: "mulligan", keep: true });
  for (let n = 0; n < 16 && f.state.step !== "main1"; n++) keywordSourceAnswer(f, { kind: "pass" });
  ...
  for (const [actor, hand] of [
    ...
    for (const name of hand) {
      const entry = keywordSourceObject(f, actor, name);
      if (entry.zone !== "hand" && !entry.commander)
        move(f.state, entry.id, "hand", "constructed authenticated source hand");
    }
    const kept = new Set(hand.map((name) => keywordSourceCard(name).id));
    ...
    const lands: KeywordSourceFixture["lands"][string] = {};
...
export function keywordSourceCast(
  f: KeywordSourceFixture,
  actor: string,
  name: string,
  target?: string,
) {
  for (let n = 0; n < f.state.players.length && f.state.decision?.actor !== actor; n++)
    keywordSourceAnswer(f, { kind: "pass" });
  if (f.state.decision?.actor !== actor) throw new Error("Source caster lacks ordinary priority");
  const before = keywordSourceObject(f, actor, name);
  keywordSourceAnswer(f, { kind: "cast", card: before.id });
  if (target) keywordSourceAnswer(f, { kind: "target", target });
  ...
  const paid = keywordSourceAnswer(f, payment);
...
export function keywordSourceResolve(f: KeywordSourceFixture) {
  const top = f.state.stack.at(-1);
...
export function keywordSourceDrain(f: KeywordSourceFixture) {
  for (
    let n = 0;
  ...
  if (f.state.stack.length || f.state.decision?.kind !== "priority")
    throw new Error("Keyword scenario did not finish all mandatory abilities");
}
export function keywordSourceInventory(f: KeywordSourceFixture, actor: string) {
...
export function keywordSourceMain(f: KeywordSourceFixture, actor: string): void {
  for (let n = 0; n < 300; n++) {
        ...
        : d.kind === "trigger-payment"
          ...
          : d.kind === "attack"
            ...
            : d.kind === "discard"
              ...
              : { kind: "pass" };
    keywordSourceAnswer(f, response);
  }
