...
const profiles = [
  {
    commander: "Tuknir Deathlock",
    support: ["Memnite", "Darksteel Sentinel", "Giant Growth", "Flame Slash"],
  },
  {
    commander: "Pavel Maliki",
  ...
  { commander: "Riven Turnbull", support: ["Memnite", "Darksteel Sentinel", "Repulse", "Murder"] },
  {
    ...
    support: ["Memnite", "Darksteel Sentinel", "Giant Growth", "Raise the Alarm"],
  },
  {
    commander: "Arvad the Cursed",
    support: ["Memnite", "Darksteel Sentinel", "Murder", "Call to the Feast"],
  },
  {
    commander: "Tatyova, Benthic Druid",
    support: ["Memnite", "Darksteel Sentinel", "Repulse", "Giant Growth"],
  },
  {
    commander: "Yargle and Multani",
    support: ["Memnite", "Darksteel Sentinel", "Murder", "Giant Growth"],
  },
] as const;
function named(source: ContentRelease, name: string): CardDefinition {
  ...
  if (!card) throw new Error(`Missing activation fixture source: ${name}`);
  return card;
}
...
async function frozenRevisions(inputs: readonly DeckRevision[], source: ContentRelease) {
  ...
  for (const input of inputs) {
    ...
    if ((await semanticHash(body)) !== hash)
      throw new Error(`Frozen activation fixture hash mismatch: ${deck.id}`);
    validate(deck, source);
    result.push(deck);
  }
  if (!result.length || new Set(result.map((deck) => deck.id)).size !== result.length)
    throw new Error("Missing or duplicate frozen activation fixture identity");
  return result;
}
...
function selectedCards(
  ...
  const available = Object.values(source.definitions)
    .filter((card) => compatible(card, commander) && card.activatedPrograms === undefined)
    .sort(affordable);
  ...
  if (selected.length !== 60)
    throw new Error(`Insufficient legal activation fixture fillers: ${commander.name}`);
  for (const card of [...assigned, ...support])
    if (!ids.has(card.id))
      throw new Error(`Required activation fixture card omitted: ${card.name}`);
  return selected;
}
async function supplement(
  ...
  if (
    ...
    throw new Error("Missing ordinary activation fixture basic lands");
  const entries = [
  ...
  const body = {
    id: `development:ordinary-activated:${commander.oracleId}:${source.hash.slice(0, 16)}`,
...
/** Preserve prior deck revisions exactly; append eight legal profiles without inventing missing commander colors. */
export async function makeOrdinaryActivatedDecks(
  ...
  const cards = ORDINARY_ACTIVATED_PERMANENTS.map((row) => {
    const card = source.definitions[`oracle:${row.identity}`];
    if (!card || !reviewedOrdinaryActivatedDefinition(card))
      throw new Error(`Missing authenticated activation source: ${row.name}`);
    return card;
  }).sort(ordered);
  ...
  if (chosen.some((card) => !card.commanderEligible))
    throw new Error("Ineligible activation fixture commander");
  const groups = assign(cards, chosen),
    supplements: DeckRevision[] = [];
  for (const [index, profile] of profiles.entries()) {
    const commander = chosen[index],
      assigned = groups[index];
    if (!commander || !assigned) throw new Error("Missing activation fixture assignment");
  ...
  if (new Set(decks.map((deck) => deck.id)).size !== decks.length)
    throw new Error("Activation fixture identity collision");
  const coverage = cards.map((card) => {
    ...
    if (!included.length && compatibleCommanders.length)
      throw new Error(
        `Activation source omitted despite a compatible admitted commander: ${card.name}`,
      );
    return {
      definition: card.id,
      name: card.name,
      ...
      category: ORDINARY_ACTIVATED_PERMANENTS.find((row) => row.identity === card.oracleId)
        ?.category,
    };
  });
  ...
  const report = {
    schema: "ordinary-activated-decks/1",
    ...
    profiles: supplements.map((deck, index) => ({
      ...
      sourceProgramDefinitionIds: deck.entries
        .filter((entry) => source.definitions[entry.definition]?.activatedPrograms !== undefined)
        .map((entry) => entry.definition),
      supportDefinitionIds: (profiles[index]?.support ?? []).map((name) => named(source, name).id),
    })),
    executedGames: 0,
    scope:
      "Legal fixture composition only. All admitted commanders are checked against every complete source color identity. Potential mana/tap payments, targeted and self modifiers, draw/life, combat and source-lifetime interactions require separate execution. Uncovered color identities receive no legality waiver or execution claim.",
  };
