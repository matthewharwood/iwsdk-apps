...
test("the closed1192 primary registry preserves and authenticates all20 observer programs", async () => {
...
for (const mode of ["gw", "gu"] as const)
  test(`legal ${mode} observer roots retain only deck cards and exact produced token templates`, async () => {
    ...
    expect(artifact.compilerVersion).toBe("development-match-plan/11");
...
test("observer programs reject older and future execution ABIs while retaining source inspection", async () => {
  ...
  for (const processorAbi of ["commander-engine/0.14.0", "commander-engine/0.16.0"]) {
