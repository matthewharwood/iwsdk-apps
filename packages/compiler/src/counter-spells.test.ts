...
test("seven counterspell identities are authenticated against the closed1097 registry and preserve exact frozen decks", async () => {
  const { source, decks } = await fixture();
  expect(Object.keys(REVIEWED_SOURCE_BINDINGS)).toHaveLength(1097);
